...
for episode_id in range(last_episode_id, max_m_episode):
    ...
    for step_id in range(max_steps):
        ...
        if episode_id % 100 == 1 and render==True:
            ...
            frame_list.append(frame_0[:,:,::-1])
            frame_list.append(frame_1[:,:,::-1])
    ...
    if episode_id % 100 == 1 and render==True:
        ...
        for f in frame_list:
            out.write(f)
        out.release()
        # imageio.mimsave(os.path.join(ckpt_folder, str(episode_id).zfill(8) + '_' +task +'.gif'), frame_list, 'GIF', duration=0.00085)
