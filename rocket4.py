...
class Rocket(object):
    ...
    def __init__(self, max_steps, task='hover', rocket_type='falcon',
        ...
        self.g = 9.8 * 0.38   # 重力加速度 改成火星
        self.H = 50  # rocket height (meters)   火箭高度
        self.I = 1/12*self.H*self.H  # Moment of inertia  转动惯量
        self.dt = 0.05
    ...
    def create_action_table(self):
        ...
        f1 = 1.0 * self.g
        f2 = 2 * self.g
        vphi0 = 0  # Nozzle angular velocity  喷管角速度
        vphi1 = 30 / 180 * np.pi
        vphi2 = -30 / 180 * np.pi
    ...
    def step(self, action):
        ...
        ## 气流影响
        ## 上升气流
        vy_new = 1.1*vy_new + np.random.rand()
        ## 下降气流
        # vy_new = 0.9*vy_new + np.random.rand()
        ## 水平气流 顺风
        # vx_new = 1.1 * vx_new + np.random.rand()
        ## 水平气流 逆风
        vx_new = 0.9 * vx_new + np.random.rand()
        ...
        if reward>0:
            reward = reward *  self.max_steps / self.step_id
        self.I = 1/12*self.H*self.H * 0.9 * (f/self.g) * self.step_id / self.max_steps 
