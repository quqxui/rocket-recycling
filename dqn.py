...
class DQN(object):
    def __init__(self):                                                         # 定义DQN的一系列属性
        self.eval_net, self.target_net = MLP(N_STATES,N_ACTIONS).to(device), MLP(N_STATES,N_ACTIONS).to(device)                           # 利用Net创建两个神经网络: 评估网络和目标网络

        self.learn_step_counter = 0                                             # for target updating
        self.memory_counter = 0                                                 # for storing memory
        self.memory = np.zeros((MEMORY_CAPACITY, N_STATES * 2 + 2))             # 初始化记忆库，一行代表一个transition
        self.optimizer = torch.optim.RMSprop(self.eval_net.parameters(), lr=LR)   # 使用Adam优化器 (输入为评估网络的参数和学习率)
        self.loss_func = nn.MSELoss()                                           # 使用均方损失函数 (loss(xi, yi)=(xi-yi)^2)


    def choose_action(self, x):                                                 
        x = torch.unsqueeze(torch.tensor(x,dtype=torch.float,device=device), 0)
    ...
    def store_transition(self, s, a, r, s_):
        ...
        # 如果记忆库满了，便覆盖旧的数据
        index = self.memory_counter % MEMORY_CAPACITY         # 这个巧妙，通过求余 只要覆盖最前面的就好                    
    ...
    def learn(self): 
        ...
        b_s = torch.tensor(b_memory[:, :N_STATES],dtype=torch.float,device=device,)
        b_a = torch.tensor(b_memory[:, N_STATES:N_STATES+1].astype(int),dtype=torch.long,device=device)
        b_r = torch.tensor(b_memory[:, N_STATES+1:N_STATES+2],dtype=torch.float,device=device)
        b_s_ = torch.tensor(b_memory[:, -N_STATES:],dtype=torch.float,device=device)
...
for episode_id in range(last_episode_id, max_m_episode):
    ...
    for step_id in range(max_steps):
        ...
        net.store_transition(s, a, r, s_)                 # 存储样本
