...
ckpt_folder = os.path.join('./', task + '_ddqn' +'_ckpt')
...
class DQN(object):
    ...
    def learn(self):                                                            # 定义学习函数(记忆库已满后便开始学习)
        ...
        Q1_argmax = torch.argmax(q_eval_test,axis = 1)
       # q_next = self.target_net(b_s_).detach()     # detach from graph, don't backpropagate
        q_next = self.target_net(b_s_)


        ################## 两种对batch 索引的做法！
        q_update = q_next.gather(1,Q1_argmax.unsqueeze(1))
        # index = torch.arange(q_next.size(0))
        # q_update = q_next[index,Q1_argmax].unsqueeze(1)
        ###################

        q_target = b_r + GAMMA * q_update
